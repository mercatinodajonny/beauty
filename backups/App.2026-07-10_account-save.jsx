...
const userFromSb = (u) => {
  ...
  return { name, type, email: u && u.email, uid: u && u.id, username, handle: username };
};
...
export default function App() {
  ...
  // Salva le modifiche dell'account: aggiorna app + Supabase (metadati, profiles) + handle dei post
  const saveAccount = async ({name,username,phone,city}) => {
    const uid = user && user.uid;
    const uname = (username||"").toLowerCase().replace(/[^a-z0-9._]/g,"");
    const oldName = user && user.name;
    const oldHandle = user && user.username;
    // 1) aggiorna subito l'utente in app (si propaga a tutte le schermate)
    setUser(u=> u ? {...u, name:name||u.name, username:uname||u.username, handle:uname||u.handle} : u);
    // 2) aggiorna in locale gli handle/nome dei post di questo utente (Esplora e Profilo)
    ...
    // 3) persisti su Supabase (best-effort)
    try{
      const sb = await getSupabase();
      await sb.auth.updateUser({ data:{ name, username:uname } });
      if(uid){
        await sb.from("profiles").upsert({ id:uid, nome:name, username:uname||null, telefono:phone||null });
        if(uname) await sb.from("posts").update({ handle:uname }).eq("user_id",uid);
      }
      showToast("Profilo aggiornato ✅");
    }catch(e){ showToast("Salvato sul dispositivo. Cloud non raggiungibile."); }
  };
