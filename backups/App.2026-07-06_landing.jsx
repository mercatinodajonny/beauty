...
function LoginScreen({onAuth}) {
  const [view,setView] = useState("landing"); // landing | welcome | role | clientForm | business
  ...
  if (view==="landing") return wrap(
    <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"0 4px 40px"}}>
        <button onClick={()=>setView("role")} style={{width:"100%",padding:"18px 0",borderRadius:999,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:17,fontWeight:800,color:"#fff",background:"linear-gradient(90deg,#FF7AC8 0%,#B06BE6 50%,#5B7CFF 100%)",boxShadow:"0 10px 30px rgba(138,107,255,.35)",letterSpacing:.2}}>
          Registrati
        </button>
    ...
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:60,paddingBottom:24,overflowY:"auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {plans.map((p,i)=>(
            <PlanCard key={i} p={p} onChoose={()=>onAuth({name:"La tua attività",type:"pro"})}/>
          ))}
        </div>
