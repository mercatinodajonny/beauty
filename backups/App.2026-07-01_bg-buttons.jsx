...
const injectFont = () => {
  ...
  s.textContent = `
    *{box-sizing:border-box;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;-webkit-tap-highlight-color:transparent}
    ...
    /* ── Sfondo vivo Beauty — sfumature rosa/blu che si muovono lentamente, anche il bianco ── */
    @keyframes bgDrift{
      0%  {background-position:0% 0%, 100% 100%, 50% 50%;}
      25% {background-position:30% 18%, 70% 82%, 60% 40%;}
      50% {background-position:60% 40%, 40% 60%, 45% 55%;}
      75% {background-position:28% 60%, 72% 40%, 55% 45%;}
      100%{background-position:0% 0%, 100% 100%, 50% 50%;}
    }
    .bg-alive{
      background:
        radial-gradient(circle at 20% 15%, rgba(255,122,200,0.16), transparent 45%),
        radial-gradient(circle at 82% 85%, rgba(91,124,255,0.16), transparent 45%),
        radial-gradient(circle at 55% 50%, rgba(255,255,255,0.9), rgba(250,250,250,0.4) 60%, transparent 75%),
        #FAFAFA;
      background-attachment:fixed;
      background-size:200% 200%, 200% 200%, 220% 220%, auto;
      animation:bgDrift 26s ease-in-out infinite;
    }
    /* Card effetto vetro leggerissimo */
    .glass-card{
...
/* HOME CLIENTE */
/* Sfumatura CTA Beauty — blu→rosa, premium e morbida (al posto del nero pieno) */
const BTN_GRAD = "linear-gradient(135deg,#5B7CFF 0%,#8A6BFF 50%,#FF7AC8 100%)";
const BTN_SHADOW = "0 6px 18px rgba(91,124,255,.28)";
        ...
        <div style={{padding:"20px 20px 0"}}>
          <div onClick={()=>nav("cl_appts")} className="ba-lift" style={{borderRadius:24,cursor:"pointer",background:"#FFFFFF",overflow:"hidden",boxShadow:`0 8px 30px ${T.brand}14, 0 2px 8px rgba(0,0,0,.04)`,position:"relative",transition:"transform .18s cubic-bezier(.34,1.56,.64,1),box-shadow .18s ease"}}>
            ...
            <div style={{padding:"12px 16px 14px"}}>
              <button onClick={e=>{e.stopPropagation();nav("cl_appts");}} style={{width:"100%",padding:"12px 0",borderRadius:14,border:"none",background:BTN_GRAD,color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit",letterSpacing:.2,boxShadow:BTN_SHADOW}}>
            ...
            {searchResults.map((pro) => {
              ...
              return (
                <div key={pro.id} onClick={()=>{nav("cl_pro",pro);setQ("");setSearching(false);}} style={{display:"flex",gap:14,cursor:"pointer",alignItems:"center",padding:"14px 16px",borderRadius:18,background:"#FFFFFF",boxShadow:"0 2px 12px rgba(0,0,0,.06)"}}>
                  ...
                  <button onClick={e=>{e.stopPropagation();nav("cl_prenota",{pro});}} style={{padding:"10px 16px",borderRadius:999,border:"none",background:BTN_GRAD,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0,boxShadow:BTN_SHADOW}}>Prenota</button>
                </div>
              );
            })}
          </div>
                    ...
                    {myPros.slice(0,10).map(pro=>{
                      ...
                      return (
                        <div key={pro.id} onClick={()=>nav("cl_pro",pro)} style={{flexShrink:0,width:150,borderRadius:20,background:"#FFFFFF",boxShadow:"0 4px 20px rgba(0,0,0,.07)",cursor:"pointer",overflow:"hidden",padding:14,display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center"}}>
                          ...
                          <button onClick={e=>{e.stopPropagation();nav("cl_prenota",{pro});}} style={{width:"100%",padding:"10px 0",borderRadius:12,border:"none",background:BTN_GRAD,color:"#fff",fontSize:12.5,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:BTN_SHADOW}}>Prenota</button>
                        </div>
                      );
                    })}
                  </div>
                    ...
                    return (
                      <div key={pro.id} style={{flexShrink:0,width:226,borderRadius:24,overflow:"hidden",background:"#FFFFFF",boxShadow:"0 6px 28px rgba(0,0,0,.09)",cursor:"pointer"}} onClick={()=>nav("cl_pro",pro)}>
                        ...
                        <div style={{padding:"14px 16px 16px"}}>
                          ...
                          <button onClick={e=>{e.stopPropagation();nav("cl_prenota",{pro});}} style={{width:"100%",padding:"12px 0",borderRadius:16,border:"none",background:BTN_GRAD,color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:BTN_SHADOW,letterSpacing:.2}}>Prenota ora</button>
                        </div>
                      </div>
                    );
                  })}
                    ...
                    {rebookPros.slice(0,4).map(pro=>{
                      ...
                      return (
                        <div key={pro.id} onClick={()=>nav("cl_pro",pro)} className="glass-card" style={{display:"flex",alignItems:"center",gap:13,padding:"12px 14px",borderRadius:20,cursor:"pointer"}}>
                          ...
                          <button onClick={e=>{e.stopPropagation();nav("cl_prenota",{pro});}} style={{padding:"10px 18px",borderRadius:999,border:"none",background:BTN_GRAD,color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit",flexShrink:0,boxShadow:BTN_SHADOW}}>Ripeti</button>
                        </div>
                      );
                    })}
                  </div>
