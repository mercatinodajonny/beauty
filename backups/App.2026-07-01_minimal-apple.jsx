...
const injectFont = () => {
  ...
  s.textContent = `
    *{box-sizing:border-box;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;-webkit-tap-highlight-color:transparent}
    ...
    /* Macchia rosa — sale/scende e si dissolve lentamente */
    .bg-alive::before{
      top:-12%; left:-18%;
      background:radial-gradient(circle, rgba(255,122,200,0.10), rgba(255,122,200,0) 70%);
      animation:blobPink 34s ease-in-out infinite;
    }
    /* Macchia blu — deriva opposta */
    .bg-alive::after{
      bottom:-12%; right:-18%;
      background:radial-gradient(circle, rgba(91,124,255,0.10), rgba(91,124,255,0) 70%);
      animation:blobBlue 42s ease-in-out infinite;
    }
    ...
    @keyframes blobPink{
      ...
      20% {opacity:.9;}
      50% {transform:translate(8vw,10vh) scale(1.15); opacity:1;}
      80% {opacity:.5;}
    ...
    @keyframes blobBlue{
      0%  {transform:translate(0,0) scale(1);        opacity:0;}
      25% {opacity:.6;}
      50% {transform:translate(-9vw,-8vh) scale(1.2); opacity:1;}
      78% {opacity:.7;}
      100%{transform:translate(0,0) scale(1);        opacity:0;}
    }
              ...
              <div style={{marginTop:24}}>
                ...
                <div style={{display:"flex",gap:10,overflowX:"auto",padding:"4px 20px 16px",WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
                  {orderedCats.map(cat => {
                    ...
                    return (
                      <button key={cat.id} onClick={()=>openCategory(cat.id)} className="glass-card"
                        style={{flexShrink:0,width:100,height:114,borderRadius:22,overflow:"hidden",position:"relative",cursor:"pointer",fontFamily:"inherit",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8}}>
                        <span style={{fontSize:38,lineHeight:1}}>{illus.big}</span>
                        <p style={{fontSize:11,fontWeight:700,color:"#111111",margin:0,textAlign:"center",lineHeight:1.2,whiteSpace:"pre-line",letterSpacing:"-.01em"}}>{cat.label}</p>
                      </button>
                    );
                  })}
                </div>
