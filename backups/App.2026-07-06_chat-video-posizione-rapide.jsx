  ...
  if (view==="businessForm") {
    ...
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:70,paddingBottom:20,overflowY:"auto"}}>
        ...
        <div style={{height:20}}/>
        <button disabled={!valid} onClick={()=>onAuth({name:form.biz.trim()||"La tua attività",type:"pro"})} style={{width:"100%",padding:"16px 0",borderRadius:14,border:"none",marginBottom:14,background:valid?"#111111":"#EDEDED",color:valid?"#fff":"#B0B0B0",fontSize:16,fontWeight:700,cursor:valid?"pointer":"default",fontFamily:"inherit",transition:"all .2s"}}>
          Crea account attività
        </button>
        <p style={{textAlign:"center",fontSize:12,color:"#B0B0B0",margin:"0 0 30px",lineHeight:1.6}}>Registrandoti accetti i <span style={{color:"#111",fontWeight:600}}>Termini</span> e la <span style={{color:"#111",fontWeight:600}}>Privacy</span></p>
      </div>
    );
...
function ClProfilo({user,onSwitch,nav,feed=FEED,favorites,setFavorites,following,setFollowing,likedPosts,setLikedPosts,onLogout,accent,setAccent,avatarConfig}) {
  ...
  // Foto profilo scelta dall'utente (stile Instagram), salvata in locale
  const [photo,setPhoto] = useState(()=>{ try{return localStorage.getItem("ba-cl-photo")||"";}catch(e){return "";} });
  const photoRef = useRef(null);
  const onPickPhoto = (e) => {
    const f = e.target.files && e.target.files[0]; e.target.value="";
    if(!f) return;
    const rd = new FileReader();
    rd.onload = () => { const url=rd.result; setPhoto(url); try{localStorage.setItem("ba-cl-photo",url);}catch(err){} };
    rd.readAsDataURL(f);
  };
...
function ProProfilo({user,onSwitch,onLogout,accent,setAccent,nav}) {
  const [photo,setPhoto] = useState(()=>{ try{return localStorage.getItem("ba-pro-photo")||"";}catch(e){return "";} });
  const photoRef = useRef(null);
  const onPickPhoto = (e) => {
    const f = e.target.files && e.target.files[0]; e.target.value="";
    if(!f) return;
    const rd = new FileReader();
    rd.onload = () => { const url=rd.result; setPhoto(url); try{localStorage.setItem("ba-pro-photo",url);}catch(err){} };
    rd.readAsDataURL(f);
  };
...
export default function App() {
  ...
  const render = () => {
    ...
    if(screen==="pro_profilo")   return <ProProfilo user={user} onSwitch={switchMode} onLogout={()=>setUser(null)} accent={accent} setAccent={setAccent} nav={nav}/>;
  ...
  return (
    <W>
      ...
      {!fullscreen && mode==="pro" && <ProFab nav={nav}/>}
...
function ProFab({nav}) {
  const [open,setOpen] = useState(false);
  const actions = [
    {icon:"📅",label:"Nuovo appuntamento",go:()=>nav("pro_agenda",{fab:"appt"})},
    {icon:"👤",label:"Nuovo cliente",go:()=>nav("pro_clienti",{fab:"client"})},
    {icon:"⏸️",label:"Blocca orario",go:()=>nav("pro_agenda",{fab:"block"})},
    {icon:"🏖️",label:"Inserisci ferie",go:()=>nav("pro_agenda",{fab:"holiday"})},
    {icon:"🛍️",label:"Nuovo servizio",go:()=>nav("pro_servizi",{fab:"service"})},
