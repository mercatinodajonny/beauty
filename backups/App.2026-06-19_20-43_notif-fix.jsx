...
const T = {
  ...
  blue:"#7B97A0",blueBg:"#DDE8EC",
  red:"#B87A6A",redBg:"#F0E0DA",
        ...
        return (
          <div key={id} data-navid={id} onClick={()=>nav(id)}
            style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",
              gap:3,padding:"8px 0 8px",borderRadius:14,cursor:"pointer",
            ...
            <span style={{fontSize:labelSize,fontWeight:active?800:600,
              color:active?T.brandDeep:T.inkSoft,pointerEvents:"none",
    ...
    <div style={{paddingBottom:110,background:T.paper,minHeight:"100dvh"}}>
      ...
      <div style={{background:T.white,paddingTop:54,paddingBottom:14,paddingLeft:20,paddingRight:20}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:4}}>
          <div>
            <h1 style={{fontSize:28,fontWeight:800,color:T.ink,margin:"0 0 2px",lineHeight:1.05,letterSpacing:"-.02em"}}>Ciao, Alessio 👋</h1>
            <p style={{fontSize:13.5,color:T.inkMid,margin:"0 0 4px",fontWeight:500}}>Pronto a prenderti cura di te?</p>
            <button onClick={()=>setShowCity(true)} style={{display:"flex",alignItems:"center",gap:5,background:"none",border:"none",padding:0,cursor:"pointer",fontFamily:"inherit"}}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.brand} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
              <span style={{fontSize:14,color:T.inkMid,fontWeight:700}}>{city}</span>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={T.inkSoft} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9l6 6 6-6"/></svg>
            </button>
          </div>
          {(()=>{
            const hasDm = conversations.some(c=>c.messages.some(m=>m.from==="pro"));
            const hasNotif = true;
            const dot = (small) => ({position:"absolute",top:small?0:-1,right:small?0:-1,width:small?9:10,height:small?9:10,borderRadius:"50%",background:"#E53935",border:"2px solid #fff"});
            return (
      ...
      {/* HERO — Trova il look che desideri (carica foto) */}
      {!(searching && q.length >= 2) && (
        <div style={{padding:"14px 20px 0",background:T.paper}}>
          <div className="clay ba-rise" onClick={()=>nav("cl_explore")} style={{position:"relative",borderRadius:22,overflow:"hidden",cursor:"pointer",minHeight:158,background:"linear-gradient(120deg,#3A2E22,#5A4632)"}}>
            <img src={MI("1522337360788-8b13dee7a37e")} alt="look" style={{position:"absolute",right:0,top:0,bottom:0,width:"52%",height:"100%",objectFit:"cover"}} onError={e=>{e.currentTarget.style.display="none";}}/>
            <div style={{position:"absolute",inset:0,background:"linear-gradient(100deg,rgba(40,30,20,.96) 38%,rgba(40,30,20,.55) 60%,rgba(40,30,20,.1) 100%)"}}/>
            <div style={{position:"relative",padding:"20px 18px",maxWidth:"66%"}}>
              <p style={{fontSize:20,fontWeight:800,color:"#fff",margin:"0 0 6px",lineHeight:1.15}}>Trova il look<br/>che desideri</p>
              <p style={{fontSize:12,color:"rgba(255,255,255,.82)",margin:"0 0 14px",lineHeight:1.35}}>Carica una foto e scopri i professionisti che possono realizzarlo</p>
              <span style={{display:"inline-flex",alignItems:"center",gap:7,background:"#fff",color:T.ink,borderRadius:99,padding:"9px 15px",fontSize:13,fontWeight:800}}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={T.ink} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>
                Carica una foto
              </span>
            </div>
          </div>
      ...
      {!(searching && q.length >= 2) && (
        <div style={{background:T.paper}}>
          ...
          {!selCat && (
            <>
              {/* Macro-categorie — card con foto di sfondo, icona + nome + n° professionisti */}
              <div style={{padding:"6px 0 4px"}}>
                <div style={{display:"flex",gap:11,overflowX:"auto",padding:"4px 20px 6px",WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
                  {MACRO_CATS.map((cat,ci) => (
                    <button key={cat.id} onClick={()=>openCategory(cat.id)} className="ba-rise ba-lift clay" style={{position:"relative",flexShrink:0,width:118,height:152,borderRadius:18,overflow:"hidden",border:"none",cursor:"pointer",padding:0,fontFamily:"inherit",background:cat.bg,animationDelay:`${0.04*ci+0.04}s`}}>
                      {/* foto di sfondo */}
                      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <MacroCatIcon id={cat.id} size={46} color="rgba(255,255,255,.5)" strokeWidth={1.4}/>
                      </div>
                      <img src={cat.img} alt={cat.label} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} onError={e=>{e.currentTarget.style.display="none";}}/>
                      {/* gradiente scuro in basso */}
                      <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(20,14,8,.82) 0%,rgba(20,14,8,.25) 45%,rgba(20,14,8,.05) 100%)"}}/>
                      {/* contenuto */}
                      <div style={{position:"absolute",left:0,right:0,bottom:0,padding:"0 11px 12px",textAlign:"left"}}>
                        <MacroCatIcon id={cat.id} size={20} color="#fff" strokeWidth={1.9}/>
                        <p style={{fontSize:13.5,fontWeight:800,color:"#fff",margin:"6px 0 1px",lineHeight:1.12,whiteSpace:"pre-line"}}>{cat.label}</p>
                        {cat.count && <p style={{fontSize:10.5,color:"rgba(255,255,255,.82)",margin:0,fontWeight:600}}>{cat.count} pro</p>}
                      </div>
                    </button>
                  ))}
                </div>
                <button onClick={()=>openCategory("altro")} style={{display:"block",margin:"4px auto 0",background:"none",border:"none",cursor:"pointer",fontSize:12.5,fontWeight:700,color:T.brand,fontFamily:"inherit"}}>Scopri altre categorie →</button>
              </div>
              ...
              {/* ── SEZIONE: Post in evidenza (feed verticale stile Explore) ── */}
              <div style={{padding:"6px 0 0"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",marginBottom:14}}>
                  <p style={{fontSize:16,fontWeight:800,color:T.ink,margin:0}}>Post in evidenza</p>
                  <button onClick={()=>nav("cl_explore")} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,fontWeight:600,color:T.brand,fontFamily:"inherit"}}>Vedi tutti</button>
                </div>
                {/* Grid 2 colonne */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,padding:"0 16px 22px"}}>
                  {FEED.map(post=>{
                    const pr=ALL_PROS.find(p=>p.id===post.proId)||ALL_PROS[0];
                    const catDef=CAT_LIST.find(c=>c.id===pr.catId)||CAT_LIST[0];
                    return (
                      <div key={post.id} onClick={()=>nav("cl_pro",pr)} className="clay" style={{borderRadius:18,overflow:"hidden",cursor:"pointer",background:catDef?.grad||T.surface,position:"relative"}}>
                        <Photo src={post.img} style={{width:"100%",aspectRatio:"1/1",display:"block"}}/>
                        <div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(to top,rgba(0,0,0,.6),transparent)",padding:"22px 8px 8px"}}>
                          <p style={{fontSize:11,fontWeight:800,color:"#fff",margin:"0 0 1px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>@{pr.handle}</p>
                          <p style={{fontSize:10,color:"rgba(255,255,255,.75)",margin:0}}>{post.cat}</p>
                        </div>
                      </div>
                    );
                  })}
              ...
              {/* ── SEZIONE: I più visitati della zona ── */}
              <div style={{background:T.white,padding:"20px 0 18px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",marginBottom:14}}>
                  <p style={{fontSize:16,fontWeight:800,color:T.ink,margin:0}}>🔥 I più visitati della zona</p>
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:0,padding:"0 20px"}}>
                  {[...prosWithDist].sort((a,b)=>(b.followers||0)-(a.followers||0)).slice(0,4).map((pro,i)=>{
                    const catDef=CAT_LIST.find(c=>c.id===pro.catId)||CAT_LIST[0];
                    const ph=proImg(pro);
                    // stima visite del mese a partire dalla popolarità (followers)
                    const visite = Math.round((pro.followers||0)*1.8 + 120);
                    return (
                      <div key={pro.id}>
                        <div onClick={()=>nav("cl_pro",pro)} style={{display:"flex",alignItems:"center",gap:13,padding:"11px 0",cursor:"pointer"}}>
                          <div style={{position:"relative",flexShrink:0}}>
                            <div style={{width:50,height:50,borderRadius:14,overflow:"hidden",background:catDef?.grad||T.surface,position:"relative"}}>
                              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><CatIcon id={pro.catId} size={22} color="rgba(255,255,255,.6)" strokeWidth={1.5}/></div>
                              {ph && <img src={ph} alt={pro.name} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} onError={e=>{e.currentTarget.style.display="none";}}/>}
                            </div>
                            {/* badge posizione classifica */}
                            <div style={{position:"absolute",top:-5,left:-5,width:20,height:20,borderRadius:"50%",background:T.brand,display:"flex",alignItems:"center",justifyContent:"center",border:"2px solid #fff"}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <p style={{fontSize:12,color:T.inkSoft,margin:"0 0 3px"}}>{pro.cat} · {pro.city}</p>
                            <div style={{display:"flex",alignItems:"center",gap:4}}>
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={T.brand} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                              <span style={{fontSize:11,color:T.inkSoft}}>{visite.toLocaleString("it-IT")} visite questo mese</span>
                            </div>
                          </div>
                          <button onClick={e=>{e.stopPropagation();nav("cl_prenota",{pro});}} style={{padding:"8px 14px",borderRadius:12,border:"none",background:T.brand,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0,touchAction:"manipulation"}}>Prenota</button>
                        </div>
                        {i<3 && <div style={{height:1,background:T.line}}/>}
                      </div>
                    );
                  })}
                </div>
              ...
              {/* ── SEZIONE: Novità nella tua zona ── */}
              <div style={{padding:"6px 0 24px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",marginBottom:14}}>
                  <p style={{fontSize:16,fontWeight:800,color:T.ink,margin:0}}>🆕 Nuovi professionisti</p>
                </div>
                <div style={{display:"flex",gap:12,overflowX:"auto",padding:"0 20px",scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
                  {[...prosWithDist].sort(()=>Math.random()-.5).slice(0,5).map(pro=>{
                    const catDef=CAT_LIST.find(c=>c.id===pro.catId)||CAT_LIST[0];
                    const ph=proImg(pro);
                    return (
                      <div key={pro.id} onClick={()=>nav("cl_pro",pro)} style={{flexShrink:0,width:130,cursor:"pointer"}}>
                        <div className="clay-soft" style={{width:130,height:130,borderRadius:18,overflow:"hidden",background:catDef?.grad||T.surface,marginBottom:8,position:"relative"}}>
                          <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><CatIcon id={pro.catId} size={42} color="rgba(255,255,255,.55)" strokeWidth={1.5}/></div>
                          {ph && <img src={ph} alt={pro.name} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} onError={e=>{e.currentTarget.style.display="none";}}/>}
                          <div style={{position:"absolute",top:7,left:7,background:T.green,borderRadius:99,padding:"3px 8px"}}>
                            <span style={{fontSize:9,fontWeight:800,color:"#fff",textTransform:"uppercase",letterSpacing:.5}}>Nuovo</span>
                          </div>
                        </div>
                        <p style={{fontSize:12,fontWeight:800,color:T.ink,margin:"0 0 1px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pro.name}</p>
                        <p style={{fontSize:11,color:T.inkSoft,margin:0}}>{pro.cat}</p>
                      </div>
                    );
                  })}
                </div>
