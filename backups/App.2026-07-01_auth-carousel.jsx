...
/* Carosello introduttivo a scorrimento (swipe destra/sinistra) */
function IntroCarousel({slides, ctaLabel, onDone, onBack}) {
  ...
  return (
    <div style={{minHeight:"100dvh",display:"flex",flexDirection:"column",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"56px 22px 0"}}>
      ...
      <div ref={ref} onScroll={onScroll} style={{flex:1,display:"flex",overflowX:"auto",scrollSnapType:"x mandatory",scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
        {slides.map((s,i)=>(
          <div key={i} style={{flex:"0 0 100%",width:"100%",scrollSnapAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 34px",textAlign:"center",boxSizing:"border-box"}}>
            <div style={{width:120,height:120,borderRadius:36,background:"rgba(255,255,255,.75)",border:"1px solid #F0F0F2",boxShadow:"0 8px 30px rgba(0,0,0,.05)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:56,marginBottom:32}}>{s.icon}</div>
            <h2 style={{fontSize:26,fontWeight:900,color:"#111111",margin:"0 0 12px",letterSpacing:"-.03em",lineHeight:1.15}}>{s.title}</h2>
      ...
      {/* Dots */}
      <div style={{display:"flex",justifyContent:"center",gap:7,padding:"8px 0 20px"}}>
        {slides.map((_,i)=>(
...
function LoginScreen({onAuth}) {
  const [view,setView] = useState("welcome"); // welcome | role | clientForm | business
  const [role,setRole] = useState(null);      // "cliente" | "pro"
  const [form,setForm] = useState({nome:"",cognome:"",email:"",tel:""});
  ...
  if (view==="welcome") return wrap(
    <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",paddingBottom:20}}>
    ...
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:70}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div><label style={lbl}>Password</label><input value={form.tel} onChange={e=>up("tel",e.target.value)} type="password" placeholder="••••••••" style={inp}/></div>
          <p style={{fontSize:13,color:"#8A8A8E",margin:"2px 2px 0",textAlign:"right",fontWeight:600}}>Password dimenticata?</p>
        </div>
        <div style={{flex:1}}/>
        <button disabled={!valid} onClick={()=>onAuth({name:"Alessio",type:"cliente"})} style={{width:"100%",padding:"16px 0",borderRadius:14,border:"none",marginBottom:14,background:valid?"#111111":"#EDEDED",color:valid?"#fff":"#B0B0B0",fontSize:16,fontWeight:700,cursor:valid?"pointer":"default",fontFamily:"inherit"}}>Accedi</button>
        <p style={{textAlign:"center",fontSize:13,color:"#8A8A8E",margin:"0 0 30px"}}>Non hai un account? <span onClick={()=>setView("role")} style={{color:"#111",fontWeight:700,cursor:"pointer"}}>Registrati</span></p>
      </div>
    );
  }
  ...
  if (view==="clientIntro") return (
    <IntroCarousel onBack={()=>setView("role")} onDone={()=>setView("clientForm")} ctaLabel="Crea il mio account"
      slides={[
        {icon:"🔍",title:"Trova i migliori vicino a te",text:"Scopri saloni, barbieri, nail artist ed esteti selezionati nella tua zona."},
        {icon:"📅",title:"Prenota in pochi secondi",text:"Scegli servizio, giorno e ora. Vedi la disponibilità in tempo reale."},
        {icon:"💬",title:"Chatta e ricevi offerte",text:"Invia una foto del look che vuoi e ricevi proposte su misura dai professionisti."},
      ]}/>
  );
  ...
  if (view==="businessIntro") return (
    <IntroCarousel onBack={()=>setView("role")} onDone={()=>setView("business")} ctaLabel="Scopri i piani"
      slides={[
        {icon:"📅",title:"Agenda intelligente",text:"Gestisci appuntamenti, staff e disponibilità in tempo reale, senza sovrapposizioni."},
        {icon:"💬",title:"Chat e offerte su misura",text:"Parla con i clienti e invia proposte personalizzate in stile Vinted."},
        {icon:"👥",title:"Clienti fidelizzati",text:"Schede clienti, storico, note e promemoria automatici per farli tornare."},
        {icon:"📈",title:"Cresci con i dati",text:"Statistiche su incassi, servizi più richiesti e andamento della tua attività."},
      ]}/>
  );
  ...
  if (view==="clientForm") {
    ...
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:70}}>
        <button onClick={()=>setView("role")} style={{alignSelf:"flex-start",background:"none",border:"none",cursor:"pointer",padding:0,marginBottom:24,display:"flex",alignItems:"center",gap:6,color:"#111",fontSize:14,fontWeight:600,fontFamily:"inherit"}}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#111" strokeWidth="2.2" strokeLinecap="round"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>Indietro
        </button>
        <h1 style={{fontSize:28,fontWeight:900,color:"#111111",margin:"0 0 8px",letterSpacing:"-.04em"}}>Crea il tuo account</h1>
        <p style={{fontSize:15,color:"#8A8A8E",margin:"0 0 26px",fontWeight:400}}>Bastano pochi dati e sei subito pronto.</p>
        ...
        <div style={{flex:1}}/>
        <button disabled={!valid} onClick={()=>onAuth({name:form.nome.trim()||"Cliente",type:"cliente"})} style={{width:"100%",padding:"16px 0",borderRadius:14,border:"none",marginBottom:14,background:valid?"#111111":"#EDEDED",color:valid?"#fff":"#B0B0B0",fontSize:16,fontWeight:700,cursor:valid?"pointer":"default",fontFamily:"inherit",transition:"all .2s"}}>
  ...
  // ── 4. ATTIVITÀ — cosa offre l'app + piani ──
  if (view==="business") {
    const features = [
      {icon:"📅",t:"Agenda intelligente",s:"Gestisci appuntamenti e disponibilità in tempo reale."},
      {icon:"💬",t:"Chat e offerte",s:"Parla coi clienti e invia proposte su misura stile Vinted."},
      {icon:"👥",t:"Clienti e schede",s:"Storico, note e fidelizzazione automatica."},
      {icon:"📈",t:"Statistiche",s:"Incassi, andamento e servizi più richiesti."},
    ];
    const plans = [
      {name:"Base",price:"Gratis",sub:"Per iniziare",feat:["Agenda e prenotazioni","Profilo pubblico","Chat coi clienti"],cta:"Inizia gratis"},
      {name:"Pro",price:"29€/mese",sub:"Per crescere",feat:["Tutto di Base","Staff illimitato","Statistiche avanzate","Notifiche & promemoria"],cta:"Prova Pro",hot:true},
    ];
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:60,paddingBottom:24,overflowY:"auto"}}>
        <button onClick={()=>setView("role")} style={{alignSelf:"flex-start",background:"none",border:"none",cursor:"pointer",padding:0,marginBottom:18,display:"flex",alignItems:"center",gap:6,color:"#111",fontSize:14,fontWeight:600,fontFamily:"inherit"}}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#111" strokeWidth="2.2" strokeLinecap="round"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>Indietro
        </button>
        <h1 style={{fontSize:27,fontWeight:900,color:"#111111",margin:"0 0 8px",letterSpacing:"-.04em",lineHeight:1.15}}>Fai crescere la tua attività</h1>
        <p style={{fontSize:15,color:"#8A8A8E",margin:"0 0 22px",fontWeight:400}}>Tutto ciò che ti serve per gestire e far crescere il tuo salone.</p>

        {/* Cosa offre */}
        <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:26}}>
          {features.map((f,i)=>(
            <div key={i} style={{display:"flex",gap:13,alignItems:"flex-start",background:"rgba(255,255,255,.8)",borderRadius:18,padding:"14px 15px",border:"1px solid #F0F0F2"}}>
              <span style={{fontSize:24,lineHeight:1}}>{f.icon}</span>
              <div><p style={{fontSize:15,fontWeight:800,color:"#111",margin:"0 0 2px"}}>{f.t}</p><p style={{fontSize:12.5,color:"#8A8A8E",margin:0,lineHeight:1.4}}>{f.s}</p></div>
            </div>
          ))}
        </div>

        {/* Piani */}
        <p style={{fontSize:12,fontWeight:800,color:"#8A8A8E",textTransform:"uppercase",letterSpacing:1.2,margin:"0 0 12px 2px"}}>Scegli il piano</p>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {plans.map((p,i)=>(
            <div key={i} style={{borderRadius:22,padding:"18px 18px",background:p.hot?"#111111":"rgba(255,255,255,.85)",border:p.hot?"none":"1.5px solid #EAEAEA",position:"relative"}}>
              {p.hot && <span style={{position:"absolute",top:16,right:16,fontSize:10,fontWeight:800,color:"#111",background:"#fff",borderRadius:99,padding:"3px 10px"}}>CONSIGLIATO</span>}
              <p style={{fontSize:14,fontWeight:800,color:p.hot?"#fff":"#111",margin:"0 0 2px"}}>{p.name} <span style={{fontWeight:500,color:p.hot?"rgba(255,255,255,.6)":"#ADADAD"}}>· {p.sub}</span></p>
              <p style={{fontSize:26,fontWeight:900,color:p.hot?"#fff":"#111",margin:"0 0 12px",letterSpacing:"-.03em"}}>{p.price}</p>
              <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:16}}>
                {p.feat.map((ft,j)=>(
                  <div key={j} style={{display:"flex",alignItems:"center",gap:8}}>
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={p.hot?"#fff":"#111"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>
                    <span style={{fontSize:13,color:p.hot?"rgba(255,255,255,.9)":"#333",fontWeight:500}}>{ft}</span>
                  </div>
                ))}
              </div>
              <button onClick={()=>onAuth({name:"La tua attività",type:"pro"})} style={{width:"100%",padding:"13px 0",borderRadius:13,border:p.hot?"none":"1.5px solid #E5E5EA",background:p.hot?"#fff":"rgba(255,255,255,.9)",color:"#111",fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{p.cta}</button>
            </div>
          ))}
        </div>
