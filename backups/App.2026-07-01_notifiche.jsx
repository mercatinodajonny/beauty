      ...
      <div style={{background:"#FFFFFF",paddingTop:56,paddingBottom:0,boxShadow:"0 1px 0 rgba(0,0,0,.04)"}}>
        <div style={{padding:"12px 20px 0",display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative"}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center",zIndex:1}}>
            <div style={{position:"relative"}}>
              <button onClick={()=>nav("cl_notifiche")} style={{width:40,height:40,borderRadius:"50%",border:"1.5px solid #EFEFEF",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",background:"#FAFAFA",padding:0,boxShadow:"0 2px 8px rgba(0,0,0,.06)"}} aria-label="Notifiche">
              ...
              {unseenNotifs>0 && <span style={{position:"absolute",top:3,right:3,minWidth:16,height:16,padding:"0 4px",borderRadius:8,background:T.brand,border:"2px solid #fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,color:"#fff",boxSizing:"border-box"}}>{unseenNotifs>9?"9+":unseenNotifs}</span>}
            </div>
            {/* Tasto DM — messaggi con i professionisti */}
            <div style={{position:"relative"}}>
              <button onClick={()=>nav("cl_chats")} style={{width:40,height:40,borderRadius:"50%",border:"1.5px solid #EFEFEF",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",background:"#FAFAFA",padding:0,boxShadow:"0 2px 8px rgba(0,0,0,.06)"}} aria-label="Messaggi">
                <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="#0D0D0E" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"/></svg>
              </button>
              {unreadChats>0 && <span style={{position:"absolute",top:3,right:3,minWidth:16,height:16,padding:"0 4px",borderRadius:8,background:T.brand,border:"2px solid #fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,color:"#fff",boxSizing:"border-box"}}>{unreadChats>9?"9+":unreadChats}</span>}
            </div>
          </div>
        </div>
...
function ChatScreen({conv,role,nav,onSeen,onSendMessage,onSendOffer,onEditOffer,onAccept,onDecline}) {
  ...
  // Segna la conversazione come letta (lato cliente) all'apertura e a ogni nuovo messaggio visualizzato
  useEffect(()=>{ if(role==="client" && onSeen) onSeen(conv.messages.length); },[conv.messages.length,role]);
...
export default function App() {
  ...
  const handleAuth = ({name,type}) => {
    ...
    else { setAccent("nonspec"); nav("cl_home"); }
  };
