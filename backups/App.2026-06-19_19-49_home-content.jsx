        ...
        <div style={{background:T.paper}}>
          ...
          {!selCat && (
            <>
              ...
              {/* ── SEZIONE: I più recensiti ── */}
              <div style={{background:T.white,padding:"20px 0 18px",marginBottom:10}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",marginBottom:14}}>
                  <p style={{fontSize:16,fontWeight:800,color:T.ink,margin:0}}>⭐ I più recensiti</p>
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:0,padding:"0 20px"}}>
                  {[...prosWithDist].sort((a,b)=>b.rating-a.rating).slice(0,4).map((pro,i)=>{
                    const catDef=CAT_LIST.find(c=>c.id===pro.catId)||CAT_LIST[0];
                    const ph=proImg(pro);
                    return (
                      <div key={pro.id}>
                        <div onClick={()=>nav("cl_pro",pro)} style={{display:"flex",alignItems:"center",gap:13,padding:"11px 0",cursor:"pointer"}}>
                          <div style={{width:50,height:50,borderRadius:14,overflow:"hidden",background:catDef?.grad||T.surface,position:"relative",flexShrink:0}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <p style={{fontSize:12,color:T.inkSoft,margin:"0 0 3px"}}>{pro.cat} · {pro.city}</p>
                            <div style={{display:"flex",alignItems:"center",gap:4}}>
                              <span style={{fontSize:12,color:"#C49A3C",fontWeight:700}}>★ {pro.rating}</span>
                              <span style={{fontSize:11,color:T.inkSoft}}>({pro.reviews} recensioni)</span>
                            </div>
                          </div>
