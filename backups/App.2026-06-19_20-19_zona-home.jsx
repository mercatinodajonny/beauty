...
const MACRO_CATS = [
  {id:"capelli_barba", label:"Capelli\n& Barba", catIds:["parrucchiere","barbiere"],            color:"#5A4A3A",bg:"#EDE8E2"},
  {id:"unghie",        label:"Unghie",           catIds:["nail_artist"],                         color:"#5A4A3A",bg:"#EDE8E2"},
  {id:"estetica",      label:"Estetica",         catIds:["estetista","ciglia","makeup","laser"], color:"#5A4A3A",bg:"#EDE8E2"},
  {id:"benessere",     label:"Benessere",        catIds:["massaggio"],                           color:"#5A4A3A",bg:"#EDE8E2"},
  {id:"tattoo",        label:"Tattoo",           catIds:["tatuatore"],                           color:"#5A4A3A",bg:"#EDE8E2"},
  {id:"altro",         label:"Altro",            catIds:[],                                      color:"#5A4A3A",bg:"#EDE8E2"},
];
    ...
    <div style={{paddingBottom:110,background:T.paper,minHeight:"100dvh"}}>
      ...
      {!(searching && q.length >= 2) && (
        <div style={{background:T.paper}}>
          ...
          {!selCat && (
            <>
              {/* Macro-categorie — 6 voci con icone illustrate custom */}
              <div style={{background:T.white,padding:"16px 0 16px"}}>
                <div style={{display:"flex",gap:14,overflowX:"auto",padding:"0 20px",WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
                  {MACRO_CATS.map((cat,ci) => (
                    <button key={cat.id} onClick={()=>openCategory(cat.id)} className="ba-rise ba-lift" style={{display:"flex",flexDirection:"column",alignItems:"center",gap:7,padding:0,border:"none",background:"none",cursor:"pointer",fontFamily:"inherit",flexShrink:0,width:62,animationDelay:`${0.04*ci+0.04}s`}}>
                      <div className="clay-soft" style={{width:58,height:58,borderRadius:"50%",background:cat.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <MacroCatIcon id={cat.id} size={28} color={cat.color} strokeWidth={1.7}/>
                      </div>
                      <span style={{fontSize:10.5,fontWeight:700,color:T.inkMid,textAlign:"center",lineHeight:1.25,whiteSpace:"pre-line"}}>{cat.label}</span>
                    </button>
                  ))}
                </div>
              </div>
