...
function LoginScreen({onAuth}) {
  ...
  if (view==="business") {
    ...
    const plans = [
      {name:"Prova gratuita",price:"3 mesi gratis",sub:"Offerta di lancio",feat:["Tutte le funzioni Pro incluse","Nessuna carta richiesta","Disdici quando vuoi"],cta:"Inizia la prova gratuita",trial:true},
      {name:"Base",price:"12,99€/mese",sub:"Per iniziare",feat:["Agenda e prenotazioni","Profilo pubblico","Chat coi clienti"],cta:"Scegli Base"},
      {name:"Pro",price:"29€/mese",sub:"Per crescere",feat:["Tutto di Base","Staff illimitato","Statistiche avanzate","Notifiche & promemoria"],cta:"Scegli Pro",hot:true},
    ];
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:60,paddingBottom:24,overflowY:"auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {plans.map((p,i)=>{
            ...
            return (
            <div key={i} style={{borderRadius:22,padding:"18px 18px",background:trial?"linear-gradient(135deg,#5B7CFF,#8A6BFF 55%,#FF7AC8)":dark?"#111111":"rgba(255,255,255,.85)",border:onDark?"none":"1.5px solid #EAEAEA",position:"relative",boxShadow:trial?"0 10px 30px rgba(91,124,255,.30)":"none"}}>
              ...
              <p style={{fontSize:26,fontWeight:900,color:onDark?"#fff":"#111",margin:"0 0 12px",letterSpacing:"-.03em"}}>{p.price}</p>
              <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:16}}>
                {p.feat.map((ft,j)=>(
                  <div key={j} style={{display:"flex",alignItems:"center",gap:8}}>
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={onDark?"#fff":"#111"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>
                    <span style={{fontSize:13,color:onDark?"rgba(255,255,255,.92)":"#333",fontWeight:500}}>{ft}</span>
                  </div>
                ))}
              </div>
              <button onClick={()=>onAuth({name:"La tua attività",type:"pro"})} style={{width:"100%",padding:"13px 0",borderRadius:13,border:onDark?"none":"1.5px solid #E5E5EA",background:"#fff",color:trial?"#5B7CFF":"#111",fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{p.cta}</button>
            </div>
          );})}
        </div>
