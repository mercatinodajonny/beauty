...
function ClProfilo({user,onSwitch,nav,feed=FEED,favorites,setFavorites,following,setFollowing,likedPosts,setLikedPosts,onLogout,accent,setAccent,avatarConfig,photo:photoProp,onSavePhoto}) {
  ...
  const onPickPhoto = async (e) => {
    ...
    rd.readAsDataURL(f);
    // 2) prova a caricarla su Supabase Storage (se pronto) e usa l'URL
    if(onSavePhoto){
      setPhotoBusy(true);
      try{ const url = await onSavePhoto(f); if(url) setPhoto(url); }
      catch(err){ /* bucket non pronto: resta la foto locale */ }
...
function ProProfilo({user,onSwitch,onLogout,accent,setAccent,nav,photo:photoProp,onSavePhoto}) {
  ...
  const onPickPhoto = async (e) => {
    ...
    if(onSavePhoto){ try{ const url = await onSavePhoto(f); if(url) setPhoto(url); }catch(err){ /* bucket non pronto: resta la foto locale */ } }
  };
...
export default function App() {
  ...
  const [feed,setFeedState] = useState(()=>{ try{ const s=localStorage.getItem("ba-feed"); return s?JSON.parse(s):FEED; }catch(e){ return FEED; } });
  const setFeed = (updater)=> setFeedState(prev=>{
    const next = typeof updater==="function" ? updater(prev) : updater;
    try{ localStorage.setItem("ba-feed", JSON.stringify(next)); }catch(e){}
    return next;
  });
  ...
  // Carica i post dal database e li unisce al feed demo
    ...
    try {
      ...
      if(data && data.length){
        const mapped = data.map(r=>({
          ...
          tags:r.tags||[], likes:r.likes||0,
          author:{handle:(r.handle||"utente"),emoji:(r.emoji||"🧑"),accent:"#8A8A8E"},
        }));
        setFeedState(prev=>{
          const seed = FEED.filter(p=>typeof p.id!=="string"); // solo i demo
          const merged = [...mapped, ...seed];
          try{ localStorage.setItem("ba-feed", JSON.stringify(merged)); }catch(e){}
          return merged;
        });
      }
  ...
  const publishPost = async (post) => {
    ...
    try {
      ...
      if(!uid) return; // non loggato con Supabase → resta salvato in locale
      ...
      const { data:ins } = await sb.from("posts").insert({ user_id:uid, image_url:imageUrl, caption:post.caption||"", cat:post.cat||null, tags:post.tags||[], handle, emoji }).select().maybeSingle();
      const newId = (ins && ins.id) || post.id;
      // aggiorna il post nel feed con URL pubblico e id del DB
      setFeed(f=>f.map(p=>p.id===post.id?{...p,id:newId,img:imageUrl,remote:true}:p));
    } catch(e){ /* upload/insert falliti: resta salvato in locale */ }
  };
