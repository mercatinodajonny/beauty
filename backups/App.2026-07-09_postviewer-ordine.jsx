...
const userFromSb = (u) => {
  ...
  const name = md.business_name || md.name || (u && u.email ? u.email.split("@")[0] : "Utente");
  return { name, type, email: u && u.email, uid: u && u.id };
};
...
function LoginScreen({onAuth,onSignup,onLogin,authBusy,authErr,authInfo,clearAuthMsg}) {
  ...
  const [form,setForm] = useState({nome:"",cognome:"",email:"",tel:"",pwd:"",biz:""});
  ...
  // ── 3. FORM CLIENTE ──
  if (view==="clientForm") {
    const valid = form.nome.trim() && form.email.trim() && form.pwd.length>=6;
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:70,paddingBottom:20,overflowY:"auto"}}>
        ...
        <div style={{height:20}}/>
        <AuthMsg/>
        <button disabled={!valid||authBusy} onClick={()=>onSignup&&onSignup({email:form.email,password:form.pwd,role:"cliente",name:form.nome.trim()||"Cliente",extra:{telefono:form.tel}})} style={{width:"100%",padding:"16px 0",borderRadius:14,border:"none",marginBottom:14,background:(valid&&!authBusy)?"#111111":"#EDEDED",color:(valid&&!authBusy)?"#fff":"#B0B0B0",fontSize:16,fontWeight:700,cursor:(valid&&!authBusy)?"pointer":"default",fontFamily:"inherit",transition:"all .2s"}}>
  ...
  if (view==="businessForm") {
    const valid = form.biz.trim() && form.email.trim() && form.pwd.length>=6;
    return wrap(
      <div className="ba-fade" style={{flex:1,display:"flex",flexDirection:"column",paddingTop:70,paddingBottom:20,overflowY:"auto"}}>
        ...
        <div style={{height:20}}/>
        <AuthMsg/>
        <button disabled={!valid||authBusy} onClick={()=>{try{localStorage.setItem("ba-pro-plan",plan||"Prova gratuita");}catch(e){} onSignup&&onSignup({email:form.email,password:form.pwd,role:"pro",name:form.nome.trim()||form.biz.trim()||"Titolare",extra:{business_name:form.biz.trim()||"La tua attività",telefono:form.tel,plan:plan||"Prova gratuita"}});}} style={{width:"100%",padding:"16px 0",borderRadius:14,border:"none",marginBottom:14,background:(valid&&!authBusy)?"#111111":"#EDEDED",color:(valid&&!authBusy)?"#fff":"#B0B0B0",fontSize:16,fontWeight:700,cursor:(valid&&!authBusy)?"pointer":"default",fontFamily:"inherit",transition:"all .2s"}}>
...
/* Visualizzatore post stile Instagram: foto grande, like, commenti, elimina */
function PostViewer({post,isOwner,liked,onToggleLike,onAddComment,onDelete,onClose}) {
  const [txt,setTxt] = useState("");
  const comments = post.comments||[];
  const likeCount = (post.likes||0)+(liked?1:0);
  return createPortal(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:1200,display:"flex",flexDirection:"column"}}>
      {/* Top bar */}
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"calc(14px + env(safe-area-inset-top,0px)) 16px 12px",flexShrink:0}}>
        <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex"}}>
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
        </button>
        <div style={{display:"flex",alignItems:"center",gap:9,flex:1,minWidth:0}}>
          <div style={{width:34,height:34,borderRadius:"50%",background:`${(post.author&&post.author.accent)||"#888"}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{(post.author&&post.author.emoji)||"🧑"}</div>
          <p style={{fontSize:14,fontWeight:700,color:"#fff",margin:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(post.author&&(post.author.name||post.author.handle))||"Post"}</p>
        </div>
        {isOwner && <button onClick={()=>onDelete(post)} style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,.12)",border:"none",borderRadius:99,padding:"7px 13px",cursor:"pointer",fontFamily:"inherit",fontSize:12.5,fontWeight:700,color:"#FF6B6B"}}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#FF6B6B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6M10 11v6M14 11v6"/></svg>
          Elimina
        </button>}
      </div>

      {/* Foto + info scrollabile */}
      <div style={{flex:1,minHeight:0,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
        <div style={{width:"100%",background:"#000"}}>
          <img src={post.img} alt="" style={{width:"100%",display:"block",maxHeight:"64vh",objectFit:"contain"}}/>
        </div>
        {/* Azioni */}
        <div style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:18}}>
          <button onClick={()=>onToggleLike(post.id)} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:6}}>
            <svg width="26" height="26" viewBox="0 0 24 24" fill={liked?"#FF3B5C":"none"} stroke={liked?"#FF3B5C":"#fff"} strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M20.8 4.6a5.5 5.5 0 00-7.8 0L12 5.7l-1.1-1a5.5 5.5 0 00-7.8 7.8l1.1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 000-7.8z"/></svg>
            <span style={{fontSize:14,fontWeight:700,color:"#fff"}}>{likeCount}</span>
          </button>
          <div style={{display:"flex",alignItems:"center",gap:6,color:"#fff"}}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"/></svg>
            <span style={{fontSize:14,fontWeight:700}}>{comments.length}</span>
          </div>
        </div>
        {post.caption && <p style={{fontSize:14,color:"#fff",margin:"0 16px 6px",lineHeight:1.45}}><span style={{fontWeight:700,marginRight:6}}>{(post.author&&post.author.handle)||"tu"}</span>{post.caption}</p>}
        {post.tags&&post.tags.length>0 && <p style={{fontSize:12.5,color:"#7FA9FF",margin:"0 16px 10px"}}>{post.tags.map(t=><span key={t} style={{marginRight:5}}>{t}</span>)}</p>}

        {/* Commenti */}
        <div style={{borderTop:"1px solid rgba(255,255,255,.12)",margin:"6px 16px 0",paddingTop:12}}>
          <p style={{fontSize:12,fontWeight:700,color:"rgba(255,255,255,.5)",textTransform:"uppercase",letterSpacing:.8,margin:"0 0 10px"}}>Commenti</p>
          {comments.length===0 && <p style={{fontSize:13,color:"rgba(255,255,255,.4)",margin:"0 0 12px"}}>Ancora nessun commento. Scrivi il primo!</p>}
          {comments.map((c,i)=>(
            <div key={c.id||i} style={{display:"flex",gap:9,marginBottom:12}}>
              <div style={{width:30,height:30,borderRadius:"50%",background:"rgba(255,255,255,.14)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0}}>{(c.by&&c.by[0])||"🙂"}</div>
              <div><p style={{fontSize:13.5,color:"#fff",margin:0,lineHeight:1.4}}><span style={{fontWeight:700,marginRight:6}}>{c.by||"Tu"}</span>{c.text}</p></div>
            </div>
          ))}
        </div>
        <div style={{height:20}}/>
      ...
      {/* Barra commento */}
      <div style={{flexShrink:0,display:"flex",alignItems:"center",gap:10,padding:"10px 16px calc(16px + env(safe-area-inset-bottom,0px))",borderTop:"1px solid rgba(255,255,255,.12)"}}>
        <input value={txt} onChange={e=>setTxt(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&txt.trim()){onAddComment(post.id,txt.trim());setTxt("");}}} placeholder="Aggiungi un commento…" style={{flex:1,border:"none",outline:"none",background:"rgba(255,255,255,.1)",borderRadius:99,padding:"12px 16px",fontSize:14,color:"#fff",fontFamily:"inherit"}}/>
        <button onClick={()=>{if(txt.trim()){onAddComment(post.id,txt.trim());setTxt("");}}} disabled={!txt.trim()} style={{background:"none",border:"none",cursor:txt.trim()?"pointer":"default",fontSize:14,fontWeight:800,color:txt.trim()?"#7FA9FF":"rgba(255,255,255,.3)",fontFamily:"inherit"}}>Invia</button>
      </div>
    </div>,
    document.body
  );
...
function ClProfilo({user,onSwitch,nav,feed=FEED,setFeed,onDeletePost,favorites,setFavorites,following,setFollowing,likedPosts,setLikedPosts,onLogout,accent,setAccent,avatarConfig,photo:photoProp,onSavePhoto}) {
  ...
  const myPosts = (feed||[]).filter(p=>p.author && p.author.name===user.name).sort((a,b)=>_ts(b)-_ts(a));
...
function ProProfilo({user,onSwitch,onLogout,accent,setAccent,nav,photo:photoProp,onSavePhoto}) {
  ...
  const biz = (savedInfo&&savedInfo.biz) || user?.name || "La tua attività";
  const [edit,setEdit] = useState(false);
  const [tmp,setTmp] = useState({biz:biz,cat:(savedInfo&&savedInfo.cat)||"Bellezza",city:(savedInfo&&savedInfo.city)||"",email:(savedInfo&&savedInfo.email)||"",phone:(savedInfo&&savedInfo.phone)||""});
  const saveInfo = () => { try{localStorage.setItem("ba-pro-info",JSON.stringify(tmp));}catch(e){} setEdit(false); };
  const [notif,setNotif] = useState(true);
    ...
    <div style={{paddingBottom:100,background:"transparent",minHeight:"100dvh"}}>
      <div style={{background:T.white,padding:"50px 18px 18px"}}>
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <h1 style={{fontSize:20,fontWeight:800,color:T.ink,margin:"0 0 3px",letterSpacing:"-.02em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tmp.biz||biz}</h1>
      ...
      <div style={{padding:"14px 16px"}}>
        ...
        <div className="clay" style={{background:T.white,borderRadius:20,overflow:"hidden",marginBottom:12}}>
          ...
          {edit ? (
            <div style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:10}}>
              {[{k:"biz",l:"Nome attivita",ph:"Es. Salon Elite"},{k:"cat",l:"Categoria",ph:"Es. Parrucchiere"},{k:"city",l:"Citta",ph:"Es. Milano"},{k:"email",l:"Email",t:"email",ph:"info@tuosalone.com"},{k:"phone",l:"Telefono",t:"tel",ph:"333 1234567"}].map(f=>(
                <div key={f.k}><label style={{fontSize:10,fontWeight:700,color:T.inkSoft,display:"block",marginBottom:4,textTransform:"uppercase",letterSpacing:.6}}>{f.l}</label><input type={f.t||"text"} placeholder={f.ph} value={tmp[f.k]} onChange={e=>setTmp(p=>({...p,[f.k]:e.target.value}))} style={{width:"100%",padding:"9px 11px",borderRadius:8,border:`1.5px solid ${T.line}`,fontSize:14,color:T.ink,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}}/></div>
              ))}
            </div>
          ) : (
...
export default function App() {
  ...
  const loadProfile = async (sb,uid) => {
    try {
      const { data } = await sb.from("profiles").select("avatar_url").eq("id",uid).maybeSingle();
  ...
  const enterAs = (appUser) => {
    setUser(appUser); setMode(appUser.type==="pro"?"pro":"cliente");
  ...
  const render = () => {
    ...
    if(screen==="pro_profilo")   return <ProProfilo user={user} onSwitch={switchMode} onLogout={doLogout} accent={accent} setAccent={setAccent} nav={nav} photo={profilePhoto} onSavePhoto={(file)=>saveProfilePhoto(user?.uid,file)}/>;
