...
export default function App() {
  ...
  useEffect(()=>{
    ...
    getSupabase().then(async sb=>{
      ...
      if(data && data.session && data.session.user){ enterAs(userFromSb(data.session.user)); }
      const r = sb.auth.onAuthStateChange((_e,session)=>{ if(session && session.user) enterAs(userFromSb(session.user)); });
  ...
  const onSignup = async ({email,password,role,name,extra}) => {
    ...
    try{
      ...
      if(data.session && data.session.user){
        ...
        enterAs(userFromSb(data.session.user));
      } else {
  ...
  const onLogin = async ({email,password}) => {
    ...
    try{
      ...
      if(data.session && data.session.user){ enterAs(userFromSb(data.session.user)); }
    } catch(e){ setAuthErr("Connessione al database non riuscita. Riprova."); }
    finally{ setAuthBusy(false); }
  };
