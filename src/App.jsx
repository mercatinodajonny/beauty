...
const injectFont = () => {
  ...
  // Display: Fraunces (serif editoriale caratteristico) · Body: Hanken Grotesk
  ...
  s.textContent = `
    h1,h2{font-family:'Fraunces','Georgia',serif !important;letter-spacing:-.02em;font-optical-sizing:auto}
    .ba-serif{font-family:'Fraunces','Georgia',serif}
    @keyframes baRise{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}
    @keyframes baFade{from{opacity:0}to{opacity:1}}
    .ba-rise{opacity:0;animation:baRise .5s cubic-bezier(.22,.61,.36,1) forwards}
    .ba-fade{opacity:0;animation:baFade .6s ease forwards}
    .ba-lift{transition:transform .22s cubic-bezier(.22,.61,.36,1),box-shadow .22s ease}
    .ba-lift:hover{transform:translateY(-3px);box-shadow:0 14px 32px rgba(36,29,23,.13)}
    @media (prefers-reduced-motion: reduce){
      .ba-rise,.ba-fade{animation:none;opacity:1}
      .ba-lift{transition:none}
    }
  `;
  document.head.appendChild(s);
};

/* PALETTE "Atelier" — beauty editorial: bone caldo + espresso + accento terracotta, ottone per le stelle */
const T = {
  ink:"#241D17",inkMid:"#6B6157",inkSoft:"#A89E92",
  line:"#E7DDCF",surface:"#F1E9DD",white:"#FFFDFA",
  paper:"#F6F0E8",
  brand:"#C2552E",brandDeep:"#A8431F",brandBg:"#F8E9E0",
  gold:"#B68A4E",goldBg:"#F4ECDD",
  green:"#3E7C5A",greenBg:"#E6F0E8",
  blue:"#3E6F8E",blueBg:"#E9F0F4",
  red:"#B23A2E",redBg:"#F8E8E4",
  amber:"#9A6B2F",amberBg:"#F6EEDD",
  purple:"#6E4A6B",purpleBg:"#F1E9EF",
};
...
const CAT_LIST = [
  {id:"barbiere",emoji:"💈",label:"Barbiere",color:"#3E5C5A"},
  {id:"parrucchiere",emoji:"💇",label:"Parrucchiere",color:"#B06A3F"},
  {id:"nail_artist",emoji:"💅",label:"Unghie",color:"#B5566E"},
  {id:"estetista",emoji:"🧖",label:"Estetica",color:"#5E7A5A"},
  {id:"laser",emoji:"✨",label:"Laser",color:"#7A5A86"},
  {id:"tatuatore",emoji:"🖋",label:"Tattoo",color:"#4A4E6E"},
  {id:"ciglia",emoji:"👁",label:"Ciglia",color:"#9A6B2F"},
  {id:"makeup",emoji:"💄",label:"Make-up",color:"#A8456A"},
  {id:"massaggio",emoji:"💆",label:"Massaggi",color:"#6E7A45"},
];
...
const BigBtn = ({label,onClick,variant="dark",disabled,style}) => (
  <button onClick={onClick} disabled={disabled} style={{width:"100%",padding:"15px 0",borderRadius:13,border:"none",cursor:disabled?"default":"pointer",fontSize:15,fontWeight:600,background:variant==="dark"?T.brand:T.surface,color:variant==="dark"?T.white:T.ink,opacity:disabled?.35:1,fontFamily:"inherit",...style}}>{label}</button>
);
const Btn = ({label,onClick,style}) => (
  <button onClick={onClick} style={{padding:"8px 14px",borderRadius:9,border:`1.5px solid ${T.line}`,background:T.white,cursor:"pointer",fontSize:13,fontWeight:500,color:T.inkMid,fontFamily:"inherit",...style}}>{label}</button>
);
  ...
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
    <div style={{background:T.white,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:430,margin:"0 auto",padding:"18px 20px 44px",maxHeight:"90dvh",overflowY:"auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <button onClick={onClose} style={{background:T.surface,border:"none",borderRadius:"50%",width:30,height:30,cursor:"pointer",fontSize:15}}>x</button>
      </div>
      {children}
    </div>
  </div>
...
const Photo = ({src,style}) => {
  ...
  return e||!src
    ? <div style={{background:`linear-gradient(160deg,${T.brand},${T.brandDeep})`,display:"flex",alignItems:"center",justifyContent:"center",...style}}><LogoMark size={32} color="rgba(255,255,255,.9)"/></div>
    ...
    <div>
      ...
      {pros.length === 0 ? (
        ...
        <div style={{padding:"4px 20px 0",display:"flex",flexDirection:"column",gap:10}}>
          ...
          {pros.map(pro => (
            <div key={pro.id} onClick={()=>onSelectPro(pro)} style={{background:T.white,borderRadius:14,border:`1.5px solid ${T.line}`,padding:"13px 15px",cursor:"pointer",display:"flex",alignItems:"center",gap:13}}>
              <div style={{width:48,height:48,borderRadius:13,background:`${pro.accent}22`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0,position:"relative"}}>
                {pro.emoji}
                {pro.verified && <div style={{position:"absolute",bottom:-2,right:-2,width:14,height:14,borderRadius:"50%",background:T.green,border:`2px solid ${T.white}`}}/>}
              </div>
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontSize:14,fontWeight:700,color:T.ink,margin:"0 0 2px"}}>{pro.name}</p>
              ...
              <button onClick={e=>{e.stopPropagation();onBook(pro);}} style={{padding:"9px 13px",borderRadius:9,border:"none",background:T.brand,color:T.white,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>Prenota</button>
            </div>
          ))}
        </div>
      ) : (
...
function NavCl({s,nav}) {
  return (
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:T.white,borderTop:`1px solid ${T.line}`,display:"flex",zIndex:100}}>
      {[
        ...
        <button key={id} onClick={()=>nav(id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"10px 0 14px",border:"none",background:"none",cursor:"pointer"}}>
          <I a={s===id}/>
          <span style={{fontSize:9,fontWeight:s===id?700:400,color:s===id?(id==="cl_preferiti"?"#E91E8C":T.ink):T.inkSoft}}>{l}</span>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:T.white,borderTop:`1px solid ${T.line}`,display:"flex",zIndex:100}}>
      {[
        ...
        <button key={id} onClick={()=>nav(id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"10px 0 14px",border:"none",background:"none",cursor:"pointer"}}>
          <I a={s===id}/>
          <span style={{fontSize:10,fontWeight:s===id?700:400,color:s===id?T.ink:T.inkSoft}}>{l}</span>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div style={{paddingBottom:90,background:T.white,minHeight:"100dvh"}}>
      ...
      <div style={{padding:"52px 20px 16px",background:T.white,borderBottom:`1px solid ${T.line}`}}>
        ...
        {/* Barra di ricerca */}
        <div style={{display:"flex",alignItems:"center",gap:10,background:T.surface,borderRadius:14,padding:"13px 16px",border:`1.5px solid ${searching?T.ink:T.line}`}}>
      ...
      {!(searching && q.length >= 2) && banner && (
        <div style={{padding:"18px 20px 0"}}>
          <div onClick={()=>nextAppt?nav("cl_appts"):nav("cl_prenota",{pro:lastAppt.proObj})} className="ba-rise ba-lift" style={{display:"flex",alignItems:"center",gap:18,padding:"22px 22px",borderRadius:20,cursor:"pointer",background:nextAppt?T.ink:T.brand,position:"relative",overflow:"hidden",boxShadow:"0 12px 30px rgba(36,29,23,.18)",animationDelay:".05s"}}>
            <div style={{position:"absolute",inset:0,background:nextAppt?"radial-gradient(120% 120% at 88% 0%, rgba(194,85,46,.55), transparent 60%)":"radial-gradient(120% 120% at 88% 0%, rgba(255,255,255,.18), transparent 60%)",pointerEvents:"none"}}/>
            <div style={{width:60,height:60,borderRadius:16,background:"rgba(255,255,255,.16)",border:"1px solid rgba(255,255,255,.2)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,position:"relative"}}>
          ...
          {!selCat && (
            <div style={{padding:"0 20px"}}>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {CAT_LIST.map((cat,ci) => (
                  <button key={cat.id} onClick={()=>openCategory(cat.id)} className="ba-rise ba-lift" style={{display:"flex",alignItems:"center",gap:16,padding:"16px 18px",borderRadius:18,border:`1px solid ${T.line}`,background:T.white,cursor:"pointer",fontFamily:"inherit",textAlign:"left",boxShadow:"0 1px 2px rgba(36,29,23,.04)",animationDelay:`${0.04*ci+0.08}s`}}>
                    <div style={{width:50,height:50,borderRadius:14,background:`${cat.color}1A`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><CatIcon id={cat.id} size={24} color={cat.color}/></div>
                    <div style={{flex:1}}>
        ...
        <div style={{padding:"14px 20px"}}>
          {favPros.map(pro => (
            <div key={pro.id} style={{background:T.white,borderRadius:16,marginBottom:10,border:`1.5px solid ${T.line}`,overflow:"hidden"}}>
              <div style={{height:54,background:`linear-gradient(135deg,${pro.accent}44,${pro.accent}18)`,display:"flex",alignItems:"center",padding:"0 16px",gap:10}}>
...
function W({children}) {
  return <div style={{maxWidth:430,margin:"0 auto",minHeight:"100dvh",background:T.paper,fontFamily:"'Hanken Grotesk',-apple-system,system-ui,sans-serif",WebkitFontSmoothing:"antialiased"}}>{children}</div>;
}
