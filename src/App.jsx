...
function ClHome({nav,favorites,setFavorites,myAppts=[],conversations=[],user,avatarConfig}) {
  ...
  // Numero di conversazioni con almeno un messaggio dal professionista
  const unreadDM = conversations.filter(c=>c.messages.some(m=>m.from==="pro")).length;
        ...
        <div style={{padding:"12px 20px 0",display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative"}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center",zIndex:1}}>
            ...
            <div style={{position:"relative"}}>
              ...
              {unreadDM>0 && <span style={{position:"absolute",top:3,right:3,minWidth:16,height:16,padding:"0 4px",borderRadius:8,background:T.brand,border:"2px solid #fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,color:"#fff",boxSizing:"border-box"}}>{unreadDM>9?"9+":unreadDM}</span>}
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  const render = () => {
    ...
    if(screen==="cl_explore")    return <ClExplore nav={nav} user={user} feed={feed} setFeed={setFeed} likedPosts={likedPosts} setLikedPosts={setLikedPosts} savedPosts={savedPosts} setSavedPosts={setSavedPosts} onSendPost={sendPostToPro}/>;
