  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:300,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.paper,borderRadius:"32px 32px 0 0",width:"100%",maxWidth:430,margin:"0 auto",maxHeight:"90dvh",display:"flex",flexDirection:"column",minHeight:0}}>
        ...
        {/* Area scrollabile interna: lo scorrimento resta nel modale, non passa allo sfondo */}
        <div style={{flex:1,minHeight:0,overflowY:"auto",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain",padding:"0 20px calc(24px + env(safe-area-inset-bottom,0px))"}}>
          {children}
        </div>
      </div>
    </div>
...
function ClHome({nav,favorites,setFavorites,myAppts=[],conversations=[],user,avatarConfig,unreadChats=0,unseenNotifs=0}) {
  ...
  const AvatarBtn = () => (
    <button onClick={()=>nav("cl_profilo")} style={{width:44,height:44,borderRadius:"50%",overflow:"hidden",border:`2.5px solid ${T.brand}`,padding:0,cursor:"pointer",background:T.brandBg,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 3px 10px ${T.brand}33`}}>
      {clPhoto
        ? <img src={clPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt="profilo"/>
        : gender==="donna"
...
function ClProfilo({user,onSwitch,nav,feed=FEED,favorites,setFavorites,following,setFollowing,likedPosts,setLikedPosts,onLogout,accent,setAccent,avatarConfig,photo:photoProp,onSavePhoto}) {
  ...
  const onPickPhoto = async (e) => {
    ...
    if(onSavePhoto){
      setPhotoBusy(true);
      try{ const url = await onSavePhoto(f); if(url) setPhoto(url); }
      catch(err){ try{ const rd2=new FileReader(); rd2.onload=()=>{try{localStorage.setItem("ba-cl-photo",rd2.result);}catch(e){}}; rd2.readAsDataURL(f);}catch(e){} }
      ...
      <div style={{background:T.white,padding:"50px 18px 0"}}>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:16}}>
          ...
          <div onClick={()=>photoRef.current&&photoRef.current.click()} style={{position:"relative",width:80,height:80,borderRadius:"50%",background:`linear-gradient(145deg,${T.brandBg},#fff)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 6px 20px ${T.brand}30`,overflow:"hidden",cursor:"pointer"}}>
            {photo
              ? <img src={photo} alt="profilo" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
        ...
        <Modal title="Nuovo appuntamento" onClose={()=>{setShowAdd(false);setSuggest([]);}}>
          <div style={{display:"flex",flexDirection:"column",gap:11}}>
            <div>
              ...
              <div style={{display:"flex",gap:7,overflowX:"auto",scrollbarWidth:"none",paddingBottom:2,WebkitOverflowScrolling:"touch"}} className="ba-noscroll">
                {upcomingDays(21).map(d=>{
                  const sel = newA.date===d.key;
                  return (
                    <button key={d.key} onClick={()=>setNewA(p=>({...p,date:d.key}))} style={{flexShrink:0,minWidth:56,padding:"8px 6px",borderRadius:12,border:`1.5px solid ${sel?T.ink:T.line}`,background:sel?T.ink:T.white,color:sel?T.white:T.inkMid,cursor:"pointer",fontFamily:"inherit",textAlign:"center",lineHeight:1.25}}>
                      <span style={{fontSize:11,fontWeight:700,display:"block"}}>{d.label}</span>
                      <span style={{fontSize:11,fontWeight:600,opacity:.85}}>{d.num} {d.mon}</span>
                    </button>
                  );
                })}
              </div>
...
function ProProfilo({user,onSwitch,onLogout,accent,setAccent,nav,photo:photoProp,onSavePhoto}) {
  ...
  const onPickPhoto = async (e) => {
    const f = e.target.files && e.target.files[0]; e.target.value="";
    if(!f) return;
    const rd = new FileReader(); rd.onload = () => setPhoto(rd.result); rd.readAsDataURL(f);
    if(onSavePhoto){ try{ const url = await onSavePhoto(f); if(url) setPhoto(url); }catch(err){ try{localStorage.setItem("ba-pro-photo",photo);}catch(e){} } }
  };
      ...
      <div style={{background:T.white,padding:"50px 18px 18px"}}>
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          ...
          <div onClick={()=>photoRef.current&&photoRef.current.click()} style={{position:"relative",width:82,height:82,borderRadius:22,background:`linear-gradient(145deg,${T.brandBg},#fff)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 6px 20px ${T.brand}30`,overflow:"hidden",cursor:"pointer"}}>
            {photo
              ? <img src={photo} alt="attivita" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
...
export default function App() {
  ...
  // Carica il profilo (foto/nome) dal database
  const loadProfile = async (sb,uid) => {
    try {
      const { data } = await sb.from("profiles").select("avatar_url").eq("id",uid).maybeSingle();
      if(data && data.avatar_url){ setProfilePhoto(data.avatar_url); try{localStorage.setItem("ba-cl-photo",data.avatar_url);}catch(e){} }
    } catch(e){}
  };
  // Carica una foto profilo su Supabase Storage e salva l'URL nel profilo
  const saveProfilePhoto = async (uid,file) => {
    ...
    setProfilePhoto(url); try{localStorage.setItem("ba-cl-photo",url);}catch(e){}
    return url;
  };
  ...
  const enterAs = (appUser) => {
    ...
    if(appUser.type==="pro"){ setAccent("blu"); nav("pro_agenda"); }
    else { try{ setAccent(localStorage.getItem("ba-accent")||"nero"); }catch(e){ setAccent("nero"); } nav("cl_home"); }
  };
